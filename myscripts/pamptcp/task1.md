...
目前仓库是 MPTCP v0.92 版本，但是我了解到 MPTCP v1（即内核中的版本）默认调度器使用的是简化版的 BLEST 算法，你能同样实现一份吗（命名你自取，可以叫做 default_v1 之类的）？确保功能，参数等一致。

然后使用新的调度算法进行测试，看看表现吗？

请你参考：https://github.com/torvalds/linux/blob/master/net/mptcp/sched.c，或者有必要你可以在 /tmp 目录克隆局部代码来参考。